            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    sql = @"insert into SMSCONFIG (SENDER_NUMBER,SENDER_CODE, PHONENETWORK, ENABLE_FLAG, USER_NAME, USER_PWD, URL, URL_PARAMS)
                            values('TEST','COOP','DTAC','1','ISOCARE','PASSWORD','http://corpsms.dtac.co.th/servlet/com.iess.socket.SmsCorplink','&RefNo={0}&Sender={1}&Msn={2}&Sno={3}&MsgType=H&Msg={4}&Encoding=25&User={5}&Password={6}')";
                    WebUtil.Query(sql);
                    }
                catch { }
                try
                ...
                try
                {
                    sql = @"insert into SMSPATTERNCONFIG (SMS_TRANS_CODE,SMS_TRANS_DESC, SMS_PATTERN, FROM_SYSTEM, ENABLE_FLAG , SMS_TRANS_SQL)
                            values('L01','อนุมัติจ่ายเงินกู้','''อนุมัติจ่ายเงินกู้ ''||loanapprove_amt_str||'' บ.รับสุทธิ ''||payoutnet_amt_str||'' บ.วันที่ ''||slip_date_str','LON','1','SMS_PATTERN_L01 where to_char(send_date,''yyyyMMdd'')=?')";
                    WebUtil.Query(sql);
                    sql = @"insert into SMSPATTERNCONFIG (SMS_TRANS_CODE,SMS_TRANS_DESC, SMS_PATTERN, FROM_SYSTEM, ENABLE_FLAG , SMS_TRANS_SQL)
                            values('L02','สัญญาเงินกู้','''สัญญาเงินกู้ ''||loancontract_no||'' ได้รับการอนุมัติแล้ว ''||loanapprove_amt_str||'' บ.วันที่ ''||loanapprove_date_str','LON','1','SMS_PATTERN_L02 where to_char(send_date,''yyyyMMdd'')=?')";
                    WebUtil.Query(sql);
                    sql = @"insert into SMSPATTERNCONFIG (SMS_TRANS_CODE,SMS_TRANS_DESC, SMS_PATTERN, FROM_SYSTEM, ENABLE_FLAG , SMS_TRANS_SQL)
                            values('D01','รับฝากเงิน','''รับฝากเงิน ''||deptslip_amt_str||'' บ.เข้าบัญชีเลขที่ ''||deptaccount_no||'' วันที่ ''||deptslip_date_str','DEP','1','SMS_PATTERN_D01 where to_char(send_date,''yyyyMMdd'')=?')";
                    WebUtil.Query(sql);
                    sql = @"insert into SMSPATTERNCONFIG (SMS_TRANS_CODE,SMS_TRANS_DESC, SMS_PATTERN, FROM_SYSTEM, ENABLE_FLAG , SMS_TRANS_SQL)
                            values('D02','ถอนเงินฝาก','''ถอนเงินฝาก ''||deptslip_amt_str||'' บ.บัญชีเลขที่ ''||deptaccount_no||'' วันที่ ''||deptslip_date_str','DEP','1','SMS_PATTERN_D02 where to_char(send_date,''yyyyMMdd'')=?')";
                    WebUtil.Query(sql);
                }
                catch { }
                try
                ...
                try
                {
                    sql = @"create or replace view SMS_PATTERN_L01 as 
                                select q.* from (
                    ...
                    sql = @"create or replace view SMS_PATTERN_L02 as 
                            ...
                            order by member_no asc ";
                    WebUtil.Query(sql);
                    sql = @"create or replace view SMS_PATTERN_D01 as 
                            select q.* from (
                            ...
                            order by member_no asc ";
                    WebUtil.Query(sql);
                    sql = @"create or replace view SMS_PATTERN_D02 as 
                            select q.* from (
                            select d.deptaccount_no
                            ,d.deptslip_date as send_date
                            ,d.deptslip_date as deptslip_date
