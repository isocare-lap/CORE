    ...
    <script type="text/javascript">
        ...
        function OnButtonClicked(s, row, oName) {
            if (oName == "b_account") {
                ...
                Gcoop.OpenIFrame2Extend("1200", "800", "w_dlg_dp_signature.aspx", "?dpdept_no=" + dpdept_no); //"918", "533"

            } else if (oName == "b_codept") {
                var dpdept_no = objDwMain.GetItem(1, "deptaccount_no");
                Gcoop.OpenDlg("918", "533", "w_dlg_dp_codept.aspx", "?dpdept_no=" + dpdept_no);
            }
            return 0;
        }
        ...
        function OnDwMainItemChanged(s, row, c, v) {
            ...
            if (c == "deptformat") {
                ...
                try {
                    ...
                    if (flag == "/") {
                        //bee
                        if (s.GetItem(1, "payfee_meth") == "1") {
                            var prncbal = 0, int_amt1 = 0, int_return = 0;
                            try{
                                prncbal = s.GetItem(1, "prncbal");
                            } catch (err) { prncbal = 0; }
                            try{ int_amt1 = s.GetItem(1, "int_amt1"); //+accuint_amt 
                            }catch (err){int_amt1 = 0;}
                            try{ int_return = s.GetItem(1, "int_return");
                            } catch (err) { int_return = 0; }
                            otherAmt = otherAmt;
                            totalNet = prncbal + int_amt1 - int_return - otherAmt;
                        } else { otherAmt = 0; }
                        //alert(prncbal + '-' + int_amt1 + '-' + int_return + '-' + otherAmt);
                        s.SetItem(1, "deptslip_netamt", totalNet);
                    }
