                ...
                while (dtContmas.Next())
                {
                    ...
                    UpdateContmaster = @"update	lncontmaster
			                set			principal_balance	= {0},
						                ...
						                principal_return	= {2},
                                        last_stm_no         = {3},
                                        interest_accum      = interest_accum - {4}
			                where	coop_id				= {5}
			                and		( loancontract_no	= {6} )
                            and member_no={7}";
                    UpdateContmaster = WebUtil.SQLFormat(UpdateContmaster, currprinc_bal, 0, 0, current_stm, interest_return, state.SsCoopId, loancontract_no, member_no);
                    ...
                    string sqlStr = @"update mbmembmaster set accum_interest = accum_interest - {0} where COOP_ID = {1} and member_no = {2}";
                    sqlStr = WebUtil.SQLFormat(sqlStr, interest_return, state.SsCoopControl, member_no);
