...
namespace Saving.Applications.shrlon.ws_sl_listpay_moneyreturn_ctrl
{
    public partial class DsList : DataSourceRepeater
    {
        ...
        public void RetrieveListRet(string member_no,string bank_code,string cash_type) 
        {
            ...
            {
                sql = @"select 
	                    mrt.coop_id,mrt.member_no,FT_GETMEMNAME(mrt.coop_id,mrt.member_no) as full_name,mrt.loancontract_no,
	                    ...
	                    mrt.entry_date
                    from (
	                    select rt.coop_id,rt.member_no,rt.loancontract_no,
                    ...
                    left join (select * from mbmembmoneytr where trtype_code='RET' and bank_code like '%" + bank_code + @"' and moneytype_code like '%" + cash_type + @"')  tr on tr.member_no = mrt.member_no and tr.coop_id = mrt.coop_id
                    group by  mrt.coop_id,mrt.member_no,mrt.loancontract_no,FT_GETMEMNAME(mrt.coop_id,mrt.member_no),tr.moneytype_code,tr.bank_code,tr.bank_branch,tr.bank_accid,mrt.entry_date
                    order by mrt.entry_date,mrt.member_no,mrt.loancontract_no";
            }
            else
            {
                 sql = @"select mrt.coop_id,mrt.member_no,FT_GETMEMNAME(mrt.coop_id,mrt.member_no) as full_name,mrt.loancontract_no,
                    sum(mrt.ret_prn) as ret_prn , sum(mrt.ret_int) as ret_int,sum(mrt.ret_prn)+sum(mrt.ret_int) as sum_return,
                    sum(mrt.seq_prn) as seq_prn,sum(mrt.seq_int) as seq_int,
                    tr.moneytype_code,tr.bank_code,tr.bank_branch,tr.bank_accid,'LON' as from_system,'RET' as returnitemtype_code,
                    mrt.entry_date
                    ...
                    left join mbmembmoneytr tr on tr.member_no = mrt.member_no and tr.coop_id = mrt.coop_id
                    where tr.trtype_code='RET'
        ...
        public void RetrieveListWrt(string member_no, string bank_code, string cash_type)
        {
            ...
            if (state.SsCoopId == "027001")
            {
                sql = @"select 
	                    ...
	                    mrt.entry_date
                    from mbmoneyreturn mrt
                    left join (select * from mbmembmoneytr where bank_code like '%" + bank_code + @"' and moneytype_code like '%" + cash_type + @"')  tron mrt.coop_id = tr.coop_id and mrt.member_no =  tr.member_no
                    where mrt.returnitemtype_code='WRT'
            ...
            if (state.SsCoopId == "027001")
            {
                sql = @"select *
                        ...
                        (
	                        ...
	                        (
		                        select 
			                        rt.coop_id,rt.member_no,rt.loancontract_no,
			                        decode(rt.returnitemtype_code,'PRN',rt.return_amount,0) as ret_prn,
			                        decode(rt.returnitemtype_code,'INT',rt.return_amount,0) as ret_int,
			                        decode(rt.returnitemtype_code,'PRN',rt.seq_no,0) as seq_prn,
			                        decode(rt.returnitemtype_code,'INT',rt.seq_no,0) as seq_int,
			                        rt.entry_date
		                        from mbmoneyreturn rt
		                        where rt.return_status = 8 and rt.returnitemtype_code in ('INT','PRN') and rt.member_no like '%" + member_no + @"%'
	                        ) mrt
	                        left join (select * from mbmembmoneytr where bank_code like '%" + bank_code + @"' and moneytype_code like '%" + cash_type + @"')  tr on mrt.member_no = tr.member_no and mrt.coop_id = tr.coop_id
                            group by  mrt.coop_id,mrt.member_no,mrt.loancontract_no,FT_GETMEMNAME(mrt.coop_id,mrt.member_no),tr.moneytype_code,tr.bank_code,tr.bank_branch,tr.bank_accid,mrt.entry_date
                        )";
            }
            else
