...
namespace Saving.Applications.shrlon.ws_sl_slip_pay_ctrl
{
    public partial class ws_sl_slip_pay : PageWebSheet, WebSheet
    {
        ...
        public void SaveWebSheet()
        {
            ...
            {
                //ทำหมายเหตุ ลำปาง
                dsMain.DATA[0].REMARK = dsMain.DATA[0].remark2;
                dsMain.DATA.Columns.Remove("remark2");
                //---//
        ...
        private void InitLnRcv()
        {
            try
            {
                ...
                try
                {
                    string sql = "select remark from mbmembmaster where coop_id={0} and member_no={1}";
                    sql = WebUtil.SQLFormat(sql, state.SsCoopId, mem_no);
                    Sdt dt = WebUtil.QuerySdt(sql);
                    if (dt.Next())
                    {
                        dsMain.DATA[0].REMARK = dt.GetString("remark");
                    }
                }
                catch { }
            }
